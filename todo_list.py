...
if "quote" not in st.session_state:
    ...
    st.session_state.quote = generate_quote("NGVRXdkwXYp+aZmUCNckvA==OntQYAZLLMiwKyFL")
...
for idx, task in enumerate(task_list):
    ...
    checked_done = task_col.checkbox(label, task.is_done, key=f"task_{idx}")
    if checked_done and not task.is_done:
        mark_done(task)
        st.rerun()
    elif not checked_done and task.is_done:
        mark_not_done(task)
        st.rerun()
